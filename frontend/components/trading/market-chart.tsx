...
export function MarketChart({
  ...
  useEffect(() => {
    ...
    const positive = css.getPropertyValue("--sk-positive").trim() || "#62bd86",
      negative = css.getPropertyValue("--sk-negative").trim() || "#e06f68",
      muted = css.getPropertyValue("--sk-muted").trim() || "#969a98",
      line = css.getPropertyValue("--sk-line").trim() || "rgba(255,255,255,.1)",
      accent = css.getPropertyValue("--sk-warning").trim() || "#d9a441";
    ...
    const comparative = comparisons.length > 0 && mode === "line";
    const priceSeries =
      mode === "candles"
        ...
        : chart.addSeries(LineSeries, {
            color: accent,
            lineWidth: 2,
            priceLineVisible: !comparative,
          });
    if (mode === "candles")
      priceSeries.setData(mapped as CandlestickData<Time>[]);
    else
      priceSeries.setData(
        mapped.map((c) => ({
          time: c.time,
          value: comparative
            ? (c.close / (mapped[0]?.close || c.close) - 1) * 100
            : c.close,
        })),
      );
    if (comparative) {
      for (const series of comparisons) {
        if (series.candles.length < 2) continue;
        const first = series.candles[0]!.close || 1;
      ...
      chart
        ...
        .setData([
          { time: mapped[0]!.time, value: 0 },
          { time: mapped.at(-1)!.time, value: 0 },
        ]);
    }
    if (reference != null) {
      ...
      ref.setData([
        { time: mapped[0]!.time, value: reference },
        { time: mapped.at(-1)!.time, value: reference },
      ]);
    }
    const volumes = comparative
      ? []
      : candles
        ...
        .map((c) => ({
          time: asTime(c.time),
          value: c.volume!,
          color: c.close >= c.open ? `${positive}55` : `${negative}55`,
        }));
    if (volumes?.length) {
    ...
    chart.subscribeCrosshairMove((p) => {
      ...
      const hit = candles.find((c) => asTime(c.time) === p.time);
      setHover(hit ?? null);
    });
  ...
  }, [candles, mode, reference, comparisons]);
  ...
  const first = candles[0]?.close ?? 0,
    current = hover ?? candles.at(-1)!;
  const change = first ? ((current.close - first) / first) * 100 : null;
  return (

...
def q(symbol: str, change: float, turnover: float, spread: float = 0.1) -> Quote:
    return Quote(provider="test", venue="TEST", symbol=symbol, asset_class=AssetClass.CRYPTO,
                 ...
                 change_24h_pct=change, turnover_24h=turnover, ts_ms=1, observed_at_ms=1)


def test_outlier_ranks_above_normal_market():
    rows = [q(f"X{i}", i * 0.1, 1_000 + i * 10) for i in range(20)]; rows.append(q("OUT", 18.0, 5_000_000))
    result = IntelligenceEngine().analyze(rows)
    assert result[0].symbol == "OUT"; assert result[0].score >= 80; assert any("движение" in reason for reason in result[0].reasons)


def test_canonical_ids_keep_provider_and_market():
    quote = q("BTCUSDT", 1, 1000); assert quote.canonical_id == "test:spot:BTCUSDT"


def test_relationship_miner_finds_lead_lag():
    from tqs_intelligence.relationships import mine_relationships
    import math
    left_prices = [100.0]; right_prices = [100.0]
    signal = [0.01 if i % 3 == 0 else -0.006 if i % 3 == 1 else 0.002 for i in range(45)]
    for i, r in enumerate(signal):
        left_prices.append(left_prices[-1] * math.exp(r)); prev_signal = signal[i-1] if i > 0 else 0.0

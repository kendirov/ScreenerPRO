...
class AccountIntelStore:
    """Durable public-account intelligence store. Private credentials are not stored here."""
    ...
    def track(self, source: str, account_id: str, label: str = '') -> TrackedAccount:
        ...
        if not source or not account_id:
            raise ValueError('source/account_id required')
        now = _now_ms()
        with self._lock:
            self._con.execute('''insert into tracked_accounts(source,account_id,label,enabled,created_at_ms)
                values(?,?,?,?,?) on conflict(source,account_id) do update set label=excluded.label, enabled=1''',
    ...
    def list_tracked(self) -> list[TrackedAccount]:
        with self._lock:
            rows = self._con.execute('select * from tracked_accounts where enabled=1 order by created_at_ms').fetchall()
    ...
    def last_fill_ts(self, source: str, account_id: str) -> int | None:
        with self._lock:
            row = self._con.execute('select max(ts_ms) from account_fills where source=? and account_id=?', [source, account_id]).fetchone()
        ...
        with self._lock:
            for x in fills:
                cur = self._con.execute('insert or ignore into account_fills values(?,?,?,?,?,?,?,?,?,?,?,?,?)', [
                    x.source, x.account_id, x.trade_id, x.ts_ms, x.symbol, x.side, x.qty, x.price, x.direction,
                    x.closed_pnl, x.fee, None if x.taker is None else int(x.taker), json.dumps(x.raw, ensure_ascii=False, default=str)
                ])
        ...
        with self._lock:
            for x in rows:
                self._con.execute('insert or replace into position_snapshots values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [
                    x.source, x.account_id, x.observed_at_ms, x.symbol, x.side, x.size, x.entry_price, x.mark_price,
                    x.notional, x.unrealized_pnl, x.leverage, x.liquidation_price, x.margin_used,
                    json.dumps(x.raw, ensure_ascii=False, default=str)
                ])
            self._con.commit()
        return len(rows)
    ...
    def recent_fills(self, source: str, account_id: str, limit: int = 2000) -> list[dict[str, Any]]:
        with self._lock:
            rows = self._con.execute('select * from account_fills where source=? and account_id=? order by ts_ms desc limit ?', [source, account_id, limit]).fetchall()
    ...
    def _profile(self, tracked: TrackedAccount) -> dict[str, Any]:
        fills = self.recent_fills(tracked.source, tracked.account_id, 5000)
        positions = self.current_positions(tracked.source, tracked.account_id, 500)
        opening = [x for x in fills if str(x.get('direction') or '').lower().startswith('open')]
        long_opens = [x for x in opening if 'long' in str(x.get('direction') or '').lower()]
        short_opens = [x for x in opening if 'short' in str(x.get('direction') or '').lower()]
        ...
        for x in fills:
            by_symbol[x['symbol']] = by_symbol.get(x['symbol'], 0.0) + abs(float(x['qty']) * float(x['price']))
        total = sum(by_symbol.values())
        top = sorted(by_symbol.items(), key=lambda z: z[1], reverse=True)[:8]
        ...
        if total and top and top[0][1] / total >= .45:
            signatures.append(f'высокая концентрация на {top[0][0]}')
        if taker and sum(1 for x in taker if x['taker']) / len(taker) >= .75:
            signatures.append('часто исполняется агрессивно (taker)')
        asc = sorted(opening, key=lambda x: x['ts_ms'])
        scale_hits = sum(1 for a, b in zip(asc, asc[1:]) if a['symbol'] == b['symbol'] and a['side'] == b['side'] and b['ts_ms'] - a['ts_ms'] <= 30 * 60_000)
        if scale_hits >= 5:
            signatures.append('часто добирает позицию сериями')
        return {
            'source': tracked.source, 'account_id': tracked.account_id, 'label': tracked.label,
            'fills': len(fills), 'open_positions': len(positions), 'symbols': len(by_symbol),
            'open_long_notional': open_long, 'open_short_notional': open_short,
            'realized_samples': len(realized), 'realized_pnl_sum': sum(realized) if realized else None,
            'realized_win_rate': (sum(1 for x in realized if x > 0) / len(realized)) if realized else None,
            'median_fill_notional': statistics.median(notionals) if notionals else None,
            'taker_share': (sum(1 for x in taker if x['taker']) / len(taker)) if taker else None,
            'long_open_share': (len(long_opens) / len(opening)) if opening else None,
            'top_instruments': [{'symbol': s, 'turnover': v, 'share': v / total if total else None} for s, v in top],
            'signatures': signatures,
            'limits_ru': [
                'Профиль описательный: он не доказывает мотив трейдера.',
                'Для оценки входов/стопов/паттернов fills должны быть синхронизированы с рыночными свечами TQS.',
                'Публичные DEX-адреса и агрегированные MOEX participant данные — разные уровни наблюдения.'
            ]
        }

    def profiles(self) -> list[dict[str, Any]]:
        return [self._profile(x) for x in self.list_tracked()]

    def profile(self, source: str, account_id: str) -> dict[str, Any] | None:
        tracked = next((x for x in self.list_tracked() if x.source == source and x.account_id == account_id), None)
...
class HyperliquidPublicAdapter:
    ...
    async def positions(self, account_id: str) -> list[PositionSnapshot]:
        payload = await self.http.post_json(self.BASE, {'type': 'clearinghouseState', 'user': account_id})
        observed = _now_ms()
        ...
        for item in payload.get('assetPositions') or []:
            ...
            if not symbol or szi == 0:
                continue
            leverage = pos.get('leverage') or {}
            lev = _f(leverage.get('value') if isinstance(leverage, dict) else leverage)
            value = _f(pos.get('positionValue'))
            out.append(PositionSnapshot(source=self.source, account_id=account_id, observed_at_ms=observed, symbol=symbol,
                side='long' if szi > 0 else 'short', size=abs(szi), entry_price=_f(pos.get('entryPx')), mark_price=None,
                notional=abs(value) if value is not None else None, unrealized_pnl=_f(pos.get('unrealizedPnl')), leverage=lev,
                liquidation_price=_f(pos.get('liquidationPx')), margin_used=_f(pos.get('marginUsed')), raw=pos))
        return out

    async def fills(self, account_id: str, start_ms: int) -> list[AccountFill]:
        payload = await self.http.post_json(self.BASE, {'type': 'userFillsByTime', 'user': account_id, 'startTime': int(start_ms), 'aggregateByTime': True})
        out: list[AccountFill] = []
        for row in payload or []:
            symbol = str(row.get('coin') or '')
            ts = int(row.get('time') or row.get('timestamp') or 0)
            px = _f(row.get('px')); qty = _f(row.get('sz'))
            if not symbol or not ts or px is None or qty is None:
                continue
            raw_id = '|'.join(str(row.get(k) or '') for k in ('hash', 'oid', 'tid', 'time', 'coin', 'side', 'px', 'sz'))
            trade_id = hashlib.sha1(raw_id.encode('utf-8')).hexdigest()
            out.append(AccountFill(source=self.source, account_id=account_id, trade_id=trade_id, ts_ms=ts, symbol=symbol,
                side='buy' if str(row.get('side') or '').upper() == 'B' else 'sell', qty=abs(qty), price=px,
                direction=str(row.get('dir') or ''), closed_pnl=_f(row.get('closedPnl')), fee=_f(row.get('fee')),
...
class AccountIntelligenceService:
    def __init__(self, control: ControlCenter, store: AccountIntelStore, http: JsonHttp, refresh_seconds: int = 60, history_days: int = 30) -> None:
        ...
        self.store = store
        self.adapter = HyperliquidPublicAdapter(http)
        self.refresh_seconds = max(15, int(refresh_seconds))
        self.history_days = max(1, min(int(history_days), 365))
    ...
    def start(self) -> None:
        if self._task is None or self._task.done():
            self._task = asyncio.create_task(self._loop(), name='tqs-account-intelligence')
    ...
    async def sync_account(self, source: str, account_id: str) -> dict[str, Any]:
        ...
        if source != 'hyperliquid':
            raise ValueError(f'unsupported account source: {source}')
        self.last_action = f'Счёт {account_id[:10]}…: позиции/fills'
        last = self.store.last_fill_ts(source, account_id)
        ...
        try:
            ...
            added = self.store.upsert_fills(fills)
            npos = self.store.save_positions(positions)
            self.store.log_sync(source, account_id, 'ok', added, npos, '')
            self.sync_count += 1
            self.last_error = None
            return {'ok': True, 'source': source, 'account_id': account_id, 'fills_added': added, 'positions': npos}
        except Exception as exc:
            self.last_error = str(exc)
            self.store.log_sync(source, account_id, 'error', 0, 0, str(exc))
            raise
        ...
        try:
            while True:
                if not self.control.get().live_allowed:
                    self.last_action = 'Счета/позиции на паузе — режим STOP'
                    await asyncio.sleep(2)
                    continue
                accounts = self.store.list_tracked()
                if not accounts:
                    self.last_action = 'Счета: нет отслеживаемых публичных адресов'
                    await asyncio.sleep(self.refresh_seconds)
                    continue
                for item in accounts:
                    try:
                        await self.sync_account(item.source, item.account_id)
                    except asyncio.CancelledError:
                        raise
                    except Exception:
                        pass
                self.last_action = f'Счета: синхронизация завершена ({len(accounts)})'
                await asyncio.sleep(self.refresh_seconds)
        finally:
            self.running = False

    def status(self) -> dict[str, Any]:
        return {'running': self.running, 'last_action': self.last_action, 'last_error': self.last_error, 'sync_count': self.sync_count, **self.store.stats()}

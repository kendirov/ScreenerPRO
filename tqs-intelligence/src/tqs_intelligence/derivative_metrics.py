...
def parse_binance_open_interest(rows: list[dict[str, Any]], canonical_id: str) -> list[MetricPoint]:
    ...
    for row in rows:
        ...
        oi = _f(row.get("sumOpenInterest"))
        oi_value = _f(row.get("sumOpenInterestValue"))
        if not ts:
            continue
        meta = {
            "symbol": row.get("symbol"),
            "history_limit": "latest_1_month",
...
def parse_binance_basis(rows: list[dict[str, Any]], canonical_id: str) -> list[MetricPoint]:
    ...
    for row in rows:
        ...
        for metric, field, unit in (
            ("basis_rate", "basisRate", "ratio"),
            ("annualized_basis_rate", "annualizedBasisRate", "ratio"),
            ("basis", "basis", "quote_currency"),
            ("basis_index_price", "indexPrice", "quote_currency"),
...
def parse_moex_futoi(block: dict[str, Any], canonical_id: str, *, delayed: bool = True) -> list[MetricPoint]:
    columns = [str(x).lower() for x in (block.get("columns") or [])]
    ...
    for row in block.get("data") or []:
        try:
            ...
            if not ts:
                continue
            raw_short = _f(row[idx["pos_short"]])
            raw_pos = _f(row[idx["pos"]]) if "pos" in idx else None
            meta = {
...
class DerivativeMetricBackfiller:
    ...
    Binance funding is requested back to the requested start where the public API
    returns data. Binance OI/basis are explicitly limited by the official public
    endpoint to roughly the latest month. MOEX FUTOI is an underlying-level
    aggregate; unauthenticated ISS is marked delayed and never presented as live.
    """
    ...
    async def _binance_paged(
        ...
        *,
        ...
        while cursor <= end_ms:
            query = dict(params)
            query.update({"startTime": cursor, "endTime": int(end_ms), "limit": int(limit)})
            ...
            if last <= cursor:
                break
            cursor = last + max(1, period_ms)
            frac = (cursor - start_ms) / max(1, end_ms - start_ms)
    ...
    async def backfill_binance(
        ...
        *,
        ...
        canonical_id = f"binance:usdt-futures:{symbol}"
        written: dict[str, int] = defaultdict(int)

        await _progress(progress, 0.02, f"Binance {symbol}: funding history")
        funding = await self._binance_paged(
            "https://fapi.binance.com/fapi/v1/fundingRate",
            {"symbol": symbol}, start_ms=start_ms, end_ms=end_ms,
            ts_field="fundingTime", limit=1000, period_ms=1,
            progress=progress, label=f"Binance {symbol} funding", lo=0.02, hi=0.34,
        )
        points = parse_binance_funding(funding, canonical_id)
        stats = self.lake.write(points)
        written["funding_rate"] += stats["rows_ingested"]

        # Official openInterestHist and basis endpoints expose only the latest
        # month. Do not imply older coverage merely because candle history is old.
        ...
        oi = await self._binance_paged(
            "https://fapi.binance.com/futures/data/openInterestHist",
            {"symbol": symbol, "period": "5m"}, start_ms=month_start, end_ms=end_ms,
            ts_field="timestamp", limit=500, period_ms=300_000,
            progress=progress, label=f"Binance {symbol} OI", lo=0.36, hi=0.67,
        )
        oi_points = parse_binance_open_interest(oi, canonical_id)
        stats = self.lake.write(oi_points)
        written["open_interest"] += stats["rows_ingested"]

        await _progress(progress, 0.69, f"Binance {symbol}: basis")
        basis = await self._binance_paged(
            "https://fapi.binance.com/futures/data/basis",
            {"pair": symbol, "contractType": "PERPETUAL", "period": "5m"},
        ...
        basis_points = parse_binance_basis(basis, canonical_id)
        stats = self.lake.write(basis_points)
        written["basis"] += stats["rows_ingested"]
        await _progress(progress, 1.0, f"Binance {symbol}: derivative metrics готовы")
        return {
            "provider": "binance", "canonical_id": canonical_id, "symbol": symbol,
            "funding_rows": len(funding), "oi_rows": len(oi), "basis_rows": len(basis),
            "points_written": dict(written),
            "coverage": {"funding_start_ms": start_ms, "oi_basis_public_limit": "latest_1_month"},
        }

    async def backfill_moex_futoi(
        self,
        *,
        symbol: str,
        start_ms: int,
        ...
        # FUTOI is aggregated by underlying (BR/Si/RI/...) across active series.
        root = str(symbol).upper()
        canonical_id = f"moex:futoi:{root}"
        start_date = datetime.fromtimestamp(start_ms / 1000, timezone.utc).date()
        end_date = datetime.fromtimestamp(end_ms / 1000, timezone.utc).date()
        cursor_date = start_date
        total_rows = total_points = pages = 0
        window_days = 7
        while cursor_date <= end_date:
            window_end = min(end_date, cursor_date + timedelta(days=window_days - 1))
            offset = 0
            while True:
                url = f"https://iss.moex.com/iss/analyticalproducts/futoi/securities/{root}.json"
                params = {
                    "from": cursor_date.isoformat(), "till": window_end.isoformat(),
                    "start": offset, "iss.meta": "off",
                }
                payload = await self.http.get_json(url, params, timeout_s=20)
                block = payload.get("futoi") if isinstance(payload, dict) else None
                if not isinstance(block, dict):
                    break
                data = block.get("data") or []
                if not data:
                    break
                points = parse_moex_futoi(block, canonical_id, delayed=not authorized)
                stats = self.lake.write(points)
                total_rows += len(data)
                total_points += stats["rows_ingested"]
                pages += 1
                offset += len(data)
                if len(data) < 1000:
                    break
                await asyncio.sleep(0.05)
            elapsed = (window_end - start_date).days + 1
        ...
        return {
            "provider": "moex", "canonical_id": canonical_id, "symbol": root,
            "rows": total_rows, "points": total_points, "pages": pages,
            "delayed": not authorized,
            "delay_note": "unauthorized ISS: 15-day delay" if not authorized else "authorized real-time",
        }
    ...
    async def backfill(self, payload: dict[str, Any], progress: Progress | None = None) -> dict[str, Any]:
        ...
        if provider == "moex":
            return await self.backfill_moex_futoi(
                symbol=str(payload["symbol"]), start_ms=start_ms, end_ms=end_ms,
                progress=progress, authorized=bool(payload.get("authorized", False)),
            )

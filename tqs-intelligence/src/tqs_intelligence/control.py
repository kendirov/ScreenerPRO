...
class ControlCenter:
        ...
        with self._lock:
            ...
            for key, value in changes.items():
                if key in data and value is not None:
                    data[key] = value
            state = ControlState(**data)
            state.mode = str(state.mode).lower()
            ...
            if state.drive_export_root:
                Path(state.drive_export_root).expanduser().mkdir(parents=True, exist_ok=True)
            self._state = state
    ...
    def status(self) -> dict[str, object]:
        state = self.get()
        lake = Path(state.data_lake_root).expanduser()
        drive = Path(state.drive_export_root).expanduser() if state.drive_export_root else None
        return {
            **asdict(state),
            'live_allowed': state.live_allowed,
            'heavy_allowed': state.heavy_allowed,
            'data_lake_exists': lake.exists(),
            'drive_export_exists': bool(drive and drive.exists()),
        }
